#!/usr/bin/env python3
"""
ROS 2 — Focus (hands on hips, ears up)
"""
...
class FocusDemo(Node):
    def __init__(self,
        ...
        self.left_arm_base = [0.0, -0.35, 0.0, -1.9, 0.0, 0.0, 0.0]
        self.right_arm_base = [0.0, 0.35, 0.0, -1.9, 0.0, 0.0, 0.0]

        self.start_time = self.get_clock().now().nanoseconds / 1e9
        self.duration = 6.0
        self.timer = self.create_timer(0.05, self.publish_anger_stream)

        self.get_logger().info('Focus avviato: mani sui fianchi ferme, orecchie dritte, per 6s')

    def _on_pointcloud(self, msg: PointCloud2) -> None:
        pass

    def publish_focus_stream(self) -> None:
        now = self.get_clock().now().nanoseconds / 1e9
        elapsed = now - self.start_time
        
        # face_msg = String()
	# face_msg.data = "THOUGHTFUL"
	# self.face_pub.publish(face_msg)

        if elapsed > self.duration:
            self.get_logger().info('Focus completato. Nodo in standby.')
            self.timer.cancel()
            return
