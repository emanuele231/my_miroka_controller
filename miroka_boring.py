...
"""
ROS 2 — "i'm boring" 
ears and neck down, neutral still arms.
"""
...
class BoringDemo(Node):
    def __init__(self,
        ...
        self.reset_ears = [0.0, 0.0]

        self.start_time = self.get_clock().now().nanoseconds / 1e9
        self.duration = 6.0
        self.timer = self.create_timer(0.05, self.publish_boring_stream)

        self.get_logger().info('"Mi sto annoiando" avviata: testa inclinata, braccia neutre, per 6s')
    ...
    def publish_boring_stream(self) -> None:
        ...
        if elapsed > self.duration:
            self.get_logger().info('"Mi sto annoiando" completata. Torno in posizione di default.')
