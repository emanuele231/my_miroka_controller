#!/usr/bin/env python3
"""
ROS 2 — Surprise (still arms, ears up)
"""
...
class SurpriseDemo(Node):
    def __init__(self,
        ...
        self.neck_pos = [0.0, 0.15, 0.0]   # testa inclinata in basso
        ...
        self.duration = 6.0
        self.timer = self.create_timer(0.05, self.publish_surprise_stream)

        self.get_logger().info('Sorpresa avviata: braccia ferme, orecchie alte, per 6s')

    def _on_pointcloud(self, msg: PointCloud2) -> None:
        pass

    def publish_surprise_stream(self) -> None:
        now = self.get_clock().now().nanoseconds / 1e9
        elapsed = now - self.start_time
        
        # face_msg = String()
	# face_msg.data = "ASTONISHED"
	# sound_msg.data = "SURPRISED_SHOUT"
	# self.face_pub.publish(face_msg)

        if elapsed > self.duration:
            self.get_logger().info('Sorpresa completata. Nodo in standby.')
